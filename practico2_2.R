...
vectores_binarios <- matrix(0, nrow = nrow(prueba), ncol = ncol(prueba) - 1)  # Inicializar matriz de vectores binarios
...
for (i in 1:) {
  P_ωi <- logaddexp(vectores_binarios[i] + log_P_Spam, probabilidades_df$Log_ham[i] + log_P_Ham)
